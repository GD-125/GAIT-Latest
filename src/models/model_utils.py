...
class ModelEnsemble:
    ...
    def predict(self, x: torch.Tensor, return_individual: bool = False) -> Tuple:
        """
        Ensemble prediction
...
class ActiveLearner:
    """Active learning utilities for continuous improvement"""
    ...
    def identify_uncertain_samples(
        ...
        Args:
            ...
            ensemble: Optional ensemble for uncertainty estimation
        ...
        with torch.no_grad():
            for batch_idx, (data, _) in enumerate(data_loader):
                if ensemble:
                    _, uncertainty = ensemble.predict_with_uncertainty(data)
                    # Use max uncertainty across classes
                    ...
                    max_uncertainty = 1.0 - max_probs
                
                # Find uncertain samples
                uncertain_mask = max_uncertainty > self.uncertainty_threshold
                batch_uncertain = torch.where(uncertain_mask)[0].cpu().numpy()
                
                # Convert to global indices
                global_indices = batch_idx * len(data) + batch_uncertain
                uncertain_indices.extend(global_indices.tolist())
    ...
    def suggest_samples_for_labeling(
        ...
        """
        Suggest samples for expert labeling
        ...
        if strategy == 'uncertainty':
            uncertain = self.identify_uncertain_samples(unlabeled_data)
            return uncertain[:n_samples]
        
        elif strategy == 'random':
            total_samples = len(unlabeled_data.dataset)
            return np.random.choice(total_samples, n_samples, replace=False).tolist()
...
if __name__ == "__main__":
    ...
    params = optimizer.count_parameters(model)
